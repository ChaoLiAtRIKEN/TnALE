...
class Individual(object):
	def __init__(self, adj_matrix=None, scope=None, **kwargs):
		...
		for i in range(1,self.dim+1):
				count=count+1
				if np.sum(np.arange(i))==self.dim:
								break

		adj_matrix_R = np.diag(size_data[0:count])
		adj_matrix_R[np.triu_indices(count, 1)] = rank
		adj_matrix_R[np.tril_indices(count, -1)] = adj_matrix_R.transpose()[np.tril_indices(count, -1)]
		...
		adj_matrix_C=np.copy(adj_matrix_R)
		adj_matrix_C[adj_matrix_C==1] = 0

		self.max_connect=np.max(np.array([np.where(adj_matrix_C[i,:]>0)[0].shape[0] for i in range(count)]))/count
		...
		self.actual_elements = np.sum([ np.prod(adj_matrix_k[d]) for d in range(count) ])
...
class Generation(object):
	def __init__(self,rse_interpolation_on,update_index,society_name,adjmatrix_set_temp,best_individual_numbers_evaluation,evaluation_numbers,local_reinitilize_update_flag,meetsamestructure_local_reupdate_flag,times_iteration_stop,local_reupdate_flag,structure_unchange_count,rse_propagate_index,rse_propagate,best_generation,times_of_LocalUpdate,times_of_LocalSampling,local_structure_update_flag,temp_best_local_structure,temp_best_local_structure_fitness,center_structure,center_structure_fitness,optimize_grid_set,n_generation,pG=None, name=None, **kwargs):
		...
		self.init_sparsity = kwargs['init_sparsity']

		self.Local_Opt_Iter =self.kwargs['Local_Opt_Iter']
		self.Rse_interpolation_on =rse_interpolation_on
		self.Rse_interpolation_Iter = self.kwargs['Rse_interpolation_Iter']
	...
	def __init_adj_matrix__(self, **kwargs):
		...
		rank = [ int(np.random.uniform()>self.init_sparsity)*np.random.randint(2, self.rank) for i in range(np.sum(np.arange(out.shape[0])))]
		rank=np.array(rank).reshape((self.size,1))
		rank[np.where(rank==0)]=1
		...
		out=np.insert(out,out.shape[0],np.zeros(self.size-out.shape[0]),0)
...
# 		self.evaluation_numbers=self.evaluation_numbers+len(optimize_grid_set[0])
	...
	def __evolve__(self):
		...
		def elimination(island,generation):
			...
			#### check if structure remains unchange in one left to right iteration (from the first rank to the permutation)
			if generation>2*(self.size-1)*self.Local_Opt_Iter+1:
				...
				if self.structure_unchange_count==self.size:
			...
			if self.local_structure_update_flag==1 or self.local_reupdate_flag==1 or self.meetsamestructure_local_reupdate_flag==1:
				...
				self.update_index=0
				if self.times_of_LocalSampling>self.Rse_interpolation_Iter and self.Rse_interpolation_on==1:
						self.Local_Step = self.kwargs['Local_Step_main']
				...
				if self.times_of_LocalSampling==self.Rse_interpolation_Iter+1 and self.Rse_interpolation_on==1:
...
# 					self.evaluation_numbers=self.evaluation_numbers+len(adj_matrix_set)
					...
					if len(island['indv'])<len(adj_matrix_set):
						...
						for i in range(len(adj_matrix_set)):
											...
											island['indv'][i].parents=('%d'%(self.best_generation)+'|'+''.join([str(self.times_of_LocalSampling)])+','+''.join([str(self.times_of_LocalUpdate)])+'|'+''.join([str(self.times_iteration_stop)])+'|'+''.join([str(self.best_individual_numbers_evaluation)]))
...
# 					self.evaluation_numbers=self.evaluation_numbers+len(adj_matrix_set)
				...
				temp1=np.arange(self.size)
				temp2=np.arange(1,self.size-1)
...
# 								temp_permutation_set.append(temp_permute.tolist())          
				...
				logging.info(self.rse_propagate_index)
				logging.info(self.rse_propagate)
...
# 				self.evaluation_numbers=self.evaluation_numbers+len(temp_optimize_grid_set[update_index])
			...
			Local_Update_Step_Num,update_position=divmod(self.times_of_LocalUpdate,2*(self.size-1)*self.Local_Opt_Iter+1)
	...
	def __evaluate__(self):
		...
		def score2rank(island, idx):
			...
			score = island['score']
			sparsity_score = [ s for s, l, c in score ]
			loss_score = [ l for s, l, c in score ]
			maximumConnect_score=[ c for s, l, c in score ]

			##### Now we have the "structure interpolation" or Rse interpolation
			...
			total_score = [ fitness_func(s, l, c) for s, l, c in zip(sparsity_score, loss_score, maximumConnect_score) ]
		...
		# RANKING
		for idx, (k, v) in enumerate(self.societies.items()):
			v['score'] = [ (indv.sparsity ,np.min(indv.repeat_loss),indv.max_connect) for indv in v['indv'] ]
			score2rank(v, idx)
...
def score_summary(obj):
	...
	for k, v in obj.societies.items():
		...
		for idx, indv in enumerate(v['indv']):
			if idx == v['rank'][0]:
				logging.info('\033[31m{} | {:.3f} | {} | {} | {:.12f} | {:.5f} | {}\033[0m'.format(indv.scope, np.log10(indv.sparsity),indv.max_connect, [ float('{:.12f}'.format(l)) for l in indv.repeat_loss ],v['estimated_rse'][idx], v['total'][idx], indv.parents))
				...
				#### turn size_data into real size_data
				count=0
				for i in range(1,dim+1):
						count=count+1
						if np.sum(np.arange(i))==dim:
										break

				adj_matrix_R = np.diag(size_data[0:count])
				adj_matrix_R[np.triu_indices(count, 1)] = rank
				adj_matrix_R[np.tril_indices(count, -1)] = adj_matrix_R.transpose()[np.tril_indices(count, -1)]
				adj_matrix_in=adj_matrix_R
				...
				logging.info('{} | {:.3f} | {} | {} | {:.12f} | {:.5f} | {}'.format(indv.scope, np.log10(indv.sparsity),indv.max_connect, [ float('{:.12f}'.format(l)) for l in indv.repeat_loss ], v['estimated_rse'][idx],v['total'][idx], indv.parents))
				logging.info(indv.adj_matrix)
...
if __name__ == '__main__':
	...
	pipeline = Overlord(		# GENERATION PROPERTIES  
													...
													N_islands=1,                  
													# INVIDUAL PROPERTIES        
													rank=5, out=np.array([4,4,4,4,4,4,4,4],dtype=int),Local_Opt_Iter=1,size=np.sum(np.arange(8)),
													Local_Step_init=int(sys.argv[2]),Local_Step_main=int(sys.argv[3]),Rse_interpolation_on=int(sys.argv[4]),
													Rse_interpolation_Iter=int(sys.argv[5]),
													rse_therhold=[1e-8],Adam_Step=[0.01],init_sparsity=0.6,
													# EVALUATION PROPERTIES 
													evaluate_repeat=2, max_iterations=40000,    
													fitness_func=[ lambda s, l, c: s+5*l+0*c], 
													#  
													# EVOLUTION PROPERTIES     
													elimiation_threshold=1,   
													# FOR COMP1.5
