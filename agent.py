...
def evaluate(tf_graph, sess, indv_scope, Adam_Step,adj_matrix,therhold,evaluate_repeat, max_iterations, evoluation_goal=None, evoluation_goal_square_norm=None):		
	with tf_graph.as_default():
		with tf.variable_scope(indv_scope):
			...
			step = tf.train.AdamOptimizer(Adam_Step).minimize(rse_loss, var_list=var_list)
			var_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=indv_scope)
...
if __name__ == '__main__':
	...
	while True:
		...
		if flag:
			...
			try:
				repeat_loss = evaluate(tf_graph=g, sess=sess, indv_scope=scope, Adam_Step=Adam_Step,adj_matrix=adj_matrix, therhold=rse_TH,evaluate_repeat=repeat, max_iterations=iters,
